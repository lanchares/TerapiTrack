...
Tras desplegar la rama \texttt{Pruebas}, la plataforma instala automáticamente las dependencias de \texttt{requirements.txt} y lanza el servidor \texttt{gunicorn}.

Las mismas variables de entorno definidas en \texttt{.env.example} (\texttt{SECRET\_KEY}, \texttt{DATABASE\_URL}, \texttt{CLOUDINARY\_*}, etc.) se configuran en Heroku mediante \texttt{heroku config:set}, adaptando \texttt{DATABASE\_URL} a la cadena de conexión de PostgreSQL.
Además, se añade el addon \texttt{heroku-postgresql} para trabajar con PostgreSQL en producción~\cite{HerokuDocs}.
...
Además de las pruebas automáticas, se utilizó \texttt{flask shell} para comprobar manualmente la integridad del modelo de datos.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{img/prueba_flask_shell.png}
