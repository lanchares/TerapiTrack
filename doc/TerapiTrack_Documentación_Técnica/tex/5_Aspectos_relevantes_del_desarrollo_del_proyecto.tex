...
\subsection{Uso de \textit{blueprints}}

Flask permite organizar las rutas en \textit{blueprints}, que son módulos independientes que después se registran en la aplicación principal.
...
Finalmente se optó por modelar ambas relaciones como muchos a muchos mediante tablas de asociación: una para vincular pacientes y profesionales, y otra para enlazar ejercicios y sesiones.
...
En la Figura~\ref{fig:login} se muestra el formulario de acceso común a todos los roles del sistema.
...
Al guardar la sesión, el sistema crea un registro en la tabla \texttt{Sesion} y registros asociados en la tabla intermedia \texttt{Ejercicio\_Sesion} para cada ejercicio incluido.
...
Cuando un paciente accede a su panel, puede ver las sesiones que tiene asignadas en las proximas tres semanas.
...
La tabla \ref{tab:flujos_uso} resume los flujos de uso más relevantes de TerapiTrack, indicando el actor principal, una descripción breve y las vistas implicadas en cada caso.

\begin{table}[H]
  \small
  \centering
  \begin{tabularx}{\textwidth}{|p{0.16\textwidth}|p{0.15\textwidth}|X|X|}
    \hline
    \textbf{Flujo} & \textbf{Actor} & \textbf{Descripción} & \textbf{Vistas implicadas} \\
    \hline
    Autenticación y acceso & Usuario (cualquier rol) &
    Introduce credenciales, accede al panel correspondiente. &
    Login, panel de administración, profesional y paciente. \\
    \hline
    Creación de sesión & Profesional &
    Selecciona paciente, elige ejercicios, fija la fecha de realización. &
    Sesiones profesional, creación de sesión y detalle de sesión. \\
    \hline
    Ejecución de sesión & Paciente &
    Consulta sesiones, sigue indicaciones profesional, graba ejercicios. &
    Mis sesiones del paciente y ejecución de sesión. \\
    \hline
    Evaluación de ejercicios & Profesional &
    Revisa grabaciones, añade puntuación y comentarios a ejercicios. &
    Sesiones, evaluación sesión, evaluación ejercicio. \\
    \hline
    Consulta historial y progreso & Paciente / profesional &
    Consultar historial sesiones, evolución de puntuaciones. &
    Progreso paciente (paciente), progreso paciente (profesional). \\
    \hline
  \end{tabularx}
  \caption{Flujos de uso principales.}
  \label{tab:flujos_uso}
\end{table}

\subsection{Visualización del progreso del paciente}

Además de almacenar las evaluaciones individuales de cada ejercicio, TerapiTrack genera gráficos de evolución temporal que muestran de forma agregada las puntuaciones obtenidas por el paciente en sus sesiones.
Estas visualizaciones permiten identificar tendencias de mejora o empeoramiento y facilitan que paciente y profesional comprendan de un vistazo cómo ha evolucionado el tratamiento a lo largo del tiempo.
...
En la Figura~\ref{fig:admin-dashboard} se muestran los indicadores básicos y los accesos directos más relevantes, mientras que desde la pantalla de configuración (Figura~\ref{fig:admin-configuracion}) se pueden ajustar aspectos como la política de retención de vídeos o el límite de almacenamiento.
...
El profesional dispone de un panel principal donde visualiza el número de pacientes asignados, sesiones pendientes y accesos a las acciones más habituales.
Desde este panel puede gestionar sus sesiones programadas (Figura~\ref{fig:prof-sesiones}), crear nuevas combinando ejercicios y fechas, y consultar el progreso de cada paciente mediante una gráfica temporal de puntuaciones generada con Chart.js (Figura~\ref{fig:prof-progreso}).
...
Finalmente, la evaluación detallada de cada ejercicio se realiza en una vista que coloca lado a lado el vídeo demostrativo y la grabación del paciente, junto con los controles para asignar una puntuación y registrar comentarios.
Esta pantalla (Figura~\ref{fig:prof-evaluar-ejercicio}) constituye el último paso del flujo de trabajo del profesional.
...
El paciente accede a un panel principal simplificado, con cuatro accesos directos a \emph{Ejercicios}, \emph{Sesiones}, \emph{Progreso} y \emph{Ayuda}, dispuesto en forma de mandos circulares fáciles de seleccionar.
...
\begin{figure}[H]
  ...
  \caption{Panel principal del paciente con accesos simplificados.}
  \label{fig:paciente-dashboard}
\end{figure}

La sección \emph{Sesiones} muestra un calendario con las próximas semanas y un resumen de la sesión seleccionada, indicando profesional, especialidad y número de ejercicios asignados.
En \emph{Ejercicios}, el paciente visualiza tarjetas con el vídeo de ejemplo, una breve descripción y la duración estimada, pudiendo reproducir cada ejercicio y conocer cuántas veces se le ha asignado.

En \emph{Progreso} se presentan indicadores numéricos (evaluaciones totales, mejor nota, media) y una gráfica temporal de las puntuaciones obtenidas a partir de las evaluaciones almacenadas, acompañadas de las tarjetas de ejercicios evaluados con los comentarios del profesional.
...
Por ejemplo, el menú del paciente incluye únicamente <<Sesiones>>, <<Ejercicios>>, <<Ayuda>> y <<Progreso>>, evitando sobrecargar la interfaz con opciones que no va a utilizar.

La plantilla base (\texttt{base.html}) define la estructura común de todas las páginas (cabecera, menú, pie de página, estilos) y se encarga de mostrar mensajes de error o confirmación de forma coherente en todo el sistema mediante \emph{flash messages} de Flask.

\subsection{Controles grandes y contraste adecuado}

En las pantallas de ejecución de sesiones se han utilizado botones grandes, con un tamaño de fuente generoso y un espaciado suficiente entre elementos para facilitar la interacción con ratón, teclado o mando SNES.
...
Durante la ejecución de una sesión, el sistema muestra de forma clara en qué ejercicio se encuentra el paciente.
...
Además, en el panel del paciente se ha habilitado el uso de un mando SNES como dispositivo de entrada alternativo, de forma que los ejercicios puedan ejecutarse con menos movimientos de precisión y con un patrón de interacción más simple que el ratón tradicional~\cite{telehealth_accessibility}.
...
Se ha implementado un conjunto de pruebas unitarias en la carpeta \texttt{tests}, que cubre las operaciones básicas de los modelos (creación, actualización, borrado y consultas) y varios de los flujos principales de los controladores.
...
En la versión final del proyecto se han definido 190 tests y se ha alcanzado una cobertura global del 99\%, con todos los modelos y las funciones auxiliares al 100\% y los controladores principales (administrador, profesional, paciente y autenticación) en torno al 99\% de líneas cubiertas.
...
En la práctica, estas pruebas permitieron detectar regresiones al refactorizar controladores y modelos, de modo que se corrigieron varios fallos de validación y de control de acceso antes de desplegar nuevas versiones.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{img/cobertura_html.png}
  \caption{Resumen de cobertura generado con \texttt{pytest}.}
...
Además de las pruebas automatizadas con \texttt{pytest}, se realizó un análisis estático del código utilizando SonarQube, como se puede ver en la figura~\ref{fig:cobertura-tests}, con el objetivo de detectar \emph{code smells}, posibles errores y problemas de mantenibilidad.
Para ello se configuró un proyecto específico para TerapiTrack, se lanzó el escaneo sobre el código de \texttt{src} y se revisaron los informes generados en la interfaz web de la herramienta.
...
\begin{figure}[H]
  ...
  \caption{Resumen del análisis estático de TerapiTrack en SonarQube.}
  \label{fig:sonarqube-resumen}
\end{figure}

\section{Retos técnicos y soluciones adoptadas}

Durante el proyecto surgieron muchos desafios técnicos, los mas significativos fueron:
\begin{enumerate}
    \item \textbf{Persistencia efímera en Heroku:} el sistema de archivos local de Heroku se reinicia diariamente, eliminando vídeos subidos. Se solucionó integrando Cloudinary para almacenamiento persistente en la nube.
    \item \textbf{Conflictos en Jinja2:} en vistas complejas, la duplicidad de etiquetas \texttt{endblock} rompía el diseño sin generar errores explícitos. Se solucionó mediante una auditoría y mostrando \textit{logs} por pantalla.
    \item \textbf{Captura de eventos de hardware:} la implementación inicial del mando con \texttt{pygame} (backend) falló en el despliegue web. La solución fue migrar la lógica al frontend usando la Gamepad API de HTML5.
\end{enumerate}