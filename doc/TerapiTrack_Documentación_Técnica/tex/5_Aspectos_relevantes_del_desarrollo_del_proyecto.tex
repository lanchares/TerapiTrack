...
La Figura~\ref{fig:cobertura-tests} resume la cobertura de pruebas unitarias obtenida con \texttt{pytest}, mostrando que los modelos y funciones auxiliares alcanzan el 100\,\% y los controladores en torno al 99\,\%~\cite{GithubTerapitrack}.
...
Además de las pruebas automatizadas con \texttt{pytest}, se realizó un análisis estático del código utilizando SonarQube como se puede ver en la figura~\ref{fig:cobertura-tests} con el objetivo de detectar \emph{code smells}, posibles errores y problemas de mantenibilidad~\cite{GithubTerapitrack}.
