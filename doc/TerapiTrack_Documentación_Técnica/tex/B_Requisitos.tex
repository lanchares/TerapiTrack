...
Estos requisitos se elaboraron durante la fase de análisis y diseño del proyecto, a partir de los objetivos generales y del contexto de uso descritos en la memoria.
A partir de ellos se ha ido tomando decisiones sobre la arquitectura y la implementación de la aplicación web, de manera que cada módulo desarrollado responde a necesidades previamente identificadas y no a decisiones improvisadas durante la programación.
...
\begin{itemize}
    ...
    \item Ofrecer una plataforma accesible y fácil de usar para personas con dificultades motoras y/o cognitivas.
...
Esta sección recoge los principales casos de uso del sistema y define cómo interactúan los actores con la aplicación para satisfacer los requisitos funcionales descritos en el catálogo anterior.
Cada caso de uso se presenta de forma estructurada, indicando sus precondiciones, flujo de acciones, postcondiciones y relación con los requisitos asociados.
...
En las Figuras~\ref{fig:casos-uso-admin}, \ref{fig:casos-uso-profesional} y \ref{fig:casos-uso-paciente} se representan los principales casos de uso del sistema agrupados por actor, lo que facilita la lectura de las relaciones y de los módulos funcionales implicados en cada rol.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{Diagrama_CasosUso_admin.png}
  \caption{Casos de uso asociados al actor Administrador.}
  \label{fig:casos-uso-admin}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{Diagrama_CasosUso_profesional.png}
  \caption{Casos de uso asociados al actor Profesional sanitario.}
  \label{fig:casos-uso-profesional}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{Diagrama_CasosUso_paciente.png}
  \caption{Casos de uso asociados al actor Paciente.}
  \label{fig:casos-uso-paciente}
\end{figure}

En las tablas~\ref{tab:cu-1-iniciar-sesion} a \ref{tab:cu-9-2-graficos-progreso} se detalla la especificación de los principales casos de uso del sistema, incluyendo precondiciones, flujo de acciones y postcondiciones asociadas a cada requisito funcional.
...
\label{tab:cu-2-1-crear-usuario}
\end{table}
...
\label{tab:cu-2-2-activar-usuario}
\end{table}
...
\label{tab:cu-4-1-crear-ejercicio}
\end{table}
...
\item Profesional: ordena por nombre o fecha de creación
...
\label{tab:cu-4-2-consultar-ejercicio}
\end{table}
...
\label{tab:cu-5-2-asignar-sesion}
\end{table}
...
\label{tab:cu-5-3-consultar-sesion}
\end{table}
...
\label{tab:cu-6-realizar-sesion}
\end{table}
...
\end{enumerate}\\
\textbf{Postcondición} & VideoRespuesta almacenado y accesible para evaluación \\
\textbf{Excepciones} & Ninguna \\
\textbf{Importancia} & Alta \\
\bottomrule
\end{tabularx}
\caption{CU-7 Grabar respuesta de ejercicio.}
\label{tab:cu-7-grabar-ejercicio}
\end{table}
...
\label{tab:cu-8-2-registrar-evaluacion}
\end{table}
...
\label{tab:cu-9-1-historial-evaluaciones}
\end{table}
...
\label{tab:cu-9-2-graficos-progreso}
\end{table}